...
RegisterNetEvent('trunk:opentrunk')
AddEventHandler('trunk:opentrunk', function()
	exports["beyond_taskbar"]:drawBar(1000,"Membuka Bagasi")
...
function openmenuvehicle()
  ...
  if not IsPedInAnyVehicle(playerPed) then
    ...
    if not Config.CheckOwnership or (Config.AllowPolice and PlayerData.job.name == "police") or (Config.CheckOwnership and myVeh) then
        ...
        if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            if locked == 1 or class == 15 or class == 16 or class == 14 then
              ...
              exports['mythic_notify']:SendAlert('error', 'This trunk is closed.')
            end
          end
        else
      ...
      exports['mythic_notify']:SendAlert('error', 'This is not your vehicle.y')
    end
  end
end
local count = 0

RegisterCommand("trunke", function()
