...
AddEventHandler(
  ...
  function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
      ...
      if targetItem.weight == -1 or ((targetItem.count + count) <= targetItem.limit) then
        TriggerEvent(
          "esx_trunk:getSharedDataStore",
          plate,
          function(store)
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('player_inv_no_space')})
      end
    end
